...
function toOption(row: TripOptionRow): TripCardData['options'][number] {
  return {
    ...
    lastCapacityCheckedAt: row.last_capacity_checked_at,
  }
}
...
function toCard(trip: TripRow, options: TripOptionRow[], ticket: TicketRow | null): TripCardData {
  return {
    trip: {
      ...
      notify: trip.notify,
      edit: trip.edit,
      lastCheckedAt: trip.last_checked_at,
      swapInProgress: trip.swap_in_progress,
    },
    options: [...options].sort((a, b) => a.priority - b.priority).map(toOption),
    ticket: ticket ? toTicket(ticket) : null,
  }
}

export function HomeClient({ pollIntervalMs }: { pollIntervalMs: number }) {
